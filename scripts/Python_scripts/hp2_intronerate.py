...
def run_hp(i):
	...
	if bait_type=="aa":
		p1=sp.Popen("%s/.local/bin/hybpiper assemble --targetfile_aa %s --readfiles %s/%s_* --prefix %s --cpu %s %s >>logs/%s.out 2>>logs/%s.err" %(username,baits,reads_dir,i,i,cputhreads,ass_options,i,i),shell=True).wait()
	else:
		p1=sp.Popen("%s/.local/bin/hybpiper assemble --targetfile_dna %s --readfiles %s/%s_* --prefix %s --cpu %s %s >>logs/%s.out 2>>logs/%s.err" %(username,baits,reads_dir,i,i,cputhreads,ass_options,i,i),shell=True).wait()		
	...
	if bait_type=="aa":
		...
		p6=sp.Popen("%s/.local/bin/hybpiper retrieve_sequences --targetfile_aa %s supercontig --sample_names %s.name --fasta_dir results/%s/%s_supercontig_seqs >>logs/%s.out 2>>logs/%s.err" %(username,baits,i,i,i,i,i),shell=True).wait() ## Harvey Added

		p13=sp.Popen("%s/.local/bin/hybpiper retrieve_sequences --targetfile_aa %s intron --sample_names %s.name --fasta_dir results/%s/%s_intron_seqs >>logs/%s.out 2>>logs/%s.err" %(username,baits,i,i,i,i,i),shell=True).wait() ## Harvey Added
		...
		p6=sp.Popen("%s/.local/bin/hybpiper retrieve_sequences --targetfile_dna %s supercontig --sample_names %s.name --fasta_dir results/%s/%s_supercontig_seqs >>logs/%s.out 2>>logs/%s.err" %(username,baits,i,i,i,i,i),shell=True).wait() ## Harvey Added

		p13=sp.Popen("%s/.local/bin/hybpiper retrieve_sequences --targetfile_dna %s intron --sample_names %s.name --fasta_dir results/%s/%s_intron_seqs >>logs/%s.out 2>>logs/%s.err" %(username,baits,i,i,i,i,i),shell=True).wait() ## Harvey Added
...
for i in filelist:
	#k=i.split("_")[0] ### Old
	k=re.split(r"_H.{8}_", i)[0] ### Harvey modification 8 May
