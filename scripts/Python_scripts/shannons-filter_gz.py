...
## Harvey Orel -- altered to output compressed fastq.gz files for paired reads, in order to conserve disk storage. (20 Oct 2022)
...
def shanpaired(i,j):
	...
	outfile1=gzip.open(outfolder+"/"+name1+".gz",'wt')
	outfile2=gzip.open(outfolder+"/"+name2+".gz",'wt')
			...
			try:
				if a[0]=="@":
					...
					if shannon1 >=shan_limit and shannon2 >=shan_limit:
						...
						outfile1.write(fq1.format("fastq"))
						outfile2.write(fq2.format("fastq"))
					#else:
