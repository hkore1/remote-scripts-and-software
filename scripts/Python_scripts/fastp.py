...
def fastp_paired(file1,file2,outfolder,keep_len,fpt):
	...
	htmlout=outfolder+str(str(file1.split("_R1")[0]).split("/")[-1])+".html"
	print(out1,out2)
	p1=subprocess.Popen("fastp -i %s -I %s -o %s -O %s -g -l %s -w %s -y -Y 60 --html %s --dedup" %(file1,file2,out1,out2,keep_len,fpt,htmlout),shell=True).wait()
